...
/* Mock PCNSL cohort matching the columns the report reads:
   "Patient ID"n, Age, Sex, ECOG_PS, EBER, os_event, os_mo.
   final_clean is the cleaned mid-term dataset the Final Report analyzes;
   we point the 'final' library at WORK and build a small stand-in so the
   report's own PROC steps run in isolation. */
libname final (work);
